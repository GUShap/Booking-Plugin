...
function getNewDepartureDateTemplate() {
    ...
    return `
    <div class="departure-date-wrapper" data-date="" data-selected="true" edit-mode="true">
        <div class="date-tab">
            <input type="date" name="departure_dates[]" value="" min="${formattedDate}" required/>
        </div>
        ...
        return `<li class="select-room-item">
...
function addNewDepartureDateTab($newDateEl) {
    const $siblings = $('.departure-date-wrapper');
    const $dateTab = $newDateEl.find('.date-tab');

    const $dateTabs = $('.departure-date-wrapper .date-tab');
    const rows = $dateTabs.toArray().reduce(function (sum, element, idx) {
        if (idx % 3 == 0 && idx != 0) sum++;
        return sum;
    }, 1);
    const accumulatedLeftOffset = $siblings.length && $siblings.length % 4 == 0
    ? 0
    : $dateTabs.last().outerWidth() + $dateTabs.last().position().left;

    const accumulatedTopOffset = (-rows * $dateTabs.first().outerHeight()) + (2 * rows);
    $dateTab.css({
        'width': `${$dateTabs.first().outerWidth()}px`,
        'left': `${accumulatedLeftOffset}px`,
        'top': `${accumulatedTopOffset}px`,
    });
}

function setDepartureDateFunctionality($dateEl) {
    const $dateInput = $dateEl.find('input[type="date"]'),
    ...
    let prev_departure_date = $dateInput.val();
    ...
    $saveBtn.on('click', function () {
        ...
        updateRetreatData({ departure_date, prev_departure_date, rooms_list, max_participants });
...
function getFormattedDepartueDate(strDate) {
    if (!strDate) return;
