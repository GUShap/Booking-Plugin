...
const addToWaitlistData = {
    action: 'add_user_to_retreat_waitlist',
    retreat_id: retreatId,
    departure_date: '',
    nonce: addToCartNonce,
    name: '',
...
const hasRetreatInCart = customVars.has_retreat_in_cart ? true : false;
const retreatItemData = customVars.retreat_item_data;
...
function setRetreatInCart() {
    ...
    const $lastDay = $selectedDay.nextAll('.day-wrapper.last').first();
    const $retreatDaysRange = $selectedDay.nextUntil('.day-wrapper[data-trip="true"].last').andSelf().add($selectedDay).add($lastDay);
    const $allDepartureDays = $('.day-wrapper[data-departure="true"]');

    renderAvailableRooms(retreatItemData, retreatItemDepartureDate, false);
    ...
    $retreatDaysRange.addClass('selected');
    $allDepartureDays.each(function () {
        const isSelectedDay = $(this).data('date') == retreatItemDepartureDate;
        if (isSelectedDay) return;
        const $unselectedTripDays = $(this).nextUntil('.day-wrapper[data-trip="true"].last').andSelf().add($(this).nextAll('.day-wrapper[data-trip="true"].last').first());
        $unselectedTripDays.addClass('full-booked');
    });
}

function setRetreatDateSelect() {
    const $tripDates = $('.retreats-calendar-content .day-wrapper[data-trip="true"]');
    const getDepartureDate = ($dayWrapper) => {
        return $dayWrapper.data('departure')
            ? $dayWrapper
            : $dayWrapper.prevAll('.day-wrapper[data-departure="true"]').first();
    }
    const getTripDays = ($dayWrapper) => {
        const $departureDate = getDepartureDate($dayWrapper);
        const $returnDate = $dayWrapper.hasClass('last')
            ? $dayWrapper
            : $dayWrapper.nextAll('.day-wrapper.last').first();

        const $allTripDays = $departureDate.nextUntil($returnDate).andSelf().add($returnDate);
        return $allTripDays;
    }
    $tripDates.on('mouseenter', function () {
        const $allTripDays = getTripDays($(this));
        // const isFullBooked = $(this).hasClass('full-booked');
        // if(isFullBooked) return;
    ...
    $tripDates.on('click', function () {
        ...
        const $allTripDays = getTripDays($(this));
        const departureDate = $allTripDays.first().data('date');
        const isFullBooked = $(this).hasClass('full-booked');
        $tripDates.removeClass('selected');
        $allTripDays.addClass('selected');
        if (isFullBooked) {
            setDepartureDateWaitList(departureDate);
        } else {
            setAvalableRooms(departureDate);
        }
    });
    $tripDates.each(function () {
        const $departureDate = getDepartureDate($(this));
        const isFullBooked = $departureDate.hasClass('full-booked');
        if (isFullBooked) {
            $(this).addClass('full-booked');
        }
    });
}
async function setAvalableRooms(departureDate) {
    const roomsData = await getAvailableRooms(retreatId, departureDate);
    if (!roomsData) return;
    renderAvailableRooms(roomsData, departureDate, true);
}
...
function renderAvailableRooms(roomsData, departureDate, triggerPopup = true) {
    ...
    for (const key in roomsData.rooms_list) {
        setRoomPopupItem($popupList, roomsData.rooms_list[key], key, retreatDuration);
        setSelectDropdownItems($roomsDropdown, roomsData.rooms_list[key], key);
    }
    setRoomSelection(roomsData);
...
function setRoomSelection(roomsData) {
    const $widgetContainer = $('.add-retreat-to-cart-container'),
        $priceWrapper = $('.room-price-wrapper'),
        $roomPrice = $priceWrapper.find('.price'),
        $roomSelectInput = $('.rooms-options'),
        $secondParticipantWrapper = $widgetContainer.find('.completing-product-wrapper[data-second-participant="true"]'),
    ...
    $roomSelectInput.on('change', function () {
        ...
        const canMultipleGuests = $selectedOption.data('can-multiple');
    ...
    $addToCartBtn.on('click', async function () {
        ...
        if (atcRes.success) {
            ...
            $anotherRoomBtn.on('click', function () {
                $afterATCoptions.fadeOut(300);
                $overlay.fadeOut(300);
                $widgetContainer.find('.completing-products input[type="checkbox"]').prop('checked', false);
                $widgetContainer.removeClass('processing');
                $('.open-list-popup-button').trigger('click');
            });
...
function setSelectDropdownItems($roomsDropdown, room, roomId) {
    ...
    const $option = $(`<option class="${isSelected ? 'selected' : ''}" value="${roomId}" data-booked="${isBooked}" data-can-multiple="${canMultipleGuests}" data-price="${price}" ${isBooked || isSelected ? "disabled" : ""}>${roomName}</option>`);
    $roomsDropdown.append($option);

}
function setRoomPopupItem($popupList, room, roomId, retreatDuration) {
    const roomName = room.room_name,
        isBooked = room.is_booked,
        isSelected = room.is_selected,
        price = +room.price,
        ...
        <div class="room-heading">
            ...
            <div class="right">
                <div class="price-wrapper">
                    ...
                    <p class="room-deposit">Deposit: $${(price * 0.15).toLocaleString()}</p>
                </div>
            </div>
        </div>
    ...
    const $bookRoomEl = isBooked
        ? $(`<p class="booked-room-alert">Booked for Current Dates</p>`)
        : $(`<button type="button" data-room="${roomId}" class="book-room-button" ${isSelected ? 'disabled' : ''}>${isSelected ? 'Selected' : 'Book Now'}</button>`);
...
function handleScroll() {
    ...
    const headerHeight = $('.header-content-wrapper').outerHeight(), // Get the height of the header
    ...
    $(window).scroll(function () {
        const scrollPosition = $(window).scrollTop(),
            heightTreshold = headerHeight + adminBarHeight;
